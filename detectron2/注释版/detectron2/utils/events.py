...
def get_event_storage():
    ...
    # * _CURRENT_STORAGE_STACK（[]） 存储了每一轮的 EventStorage 对象
...
class TensorboardXWriter(EventWriter):
    ...
    def __init__(self, log_dir: str, window_size: int = 20, **kwargs):
        ...
        Args:
            ...
            其他一些传递到`torch.utils.tensorboard.SummaryWriter(...)`的参数
        """
        self._window_size = window_size
        from torch.utils.tensorboard import SummaryWriter  # 创建writer
        # * 在初始化时，定义了一个SummaryWriter用于写入

        self._writer = SummaryWriter(log_dir, **kwargs)


    def write(self):
        storage = get_event_storage()  # 返回一个EventStorage对象
        # * 通过 storage = get_event_storage() 来获取了训练过程中的数据，并将其导入tensorboard
...
class EventStorage:
    ...
    def step(self):
        ...
        """
        self._iter += 1
        # * 每一轮都会创建一个新的 self._latest_scalars 以存储新的数据
        self._latest_scalars = {}
    ...
    def __enter__(self):
        # * 每当用with...as... 的方法来创建一个 EventStorage 对象时，就向_CURRENT_STORAGE_STACK（[]） 当中存储下当前的EventStorage 对象，而对于在程序的任何一个地方，当调用 get_event_storage 方法时，就会获取到当前的这个对象，则自然可以简单的将信息存储在这个对象中。
