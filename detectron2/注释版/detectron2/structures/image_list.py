...
class ImageList(object):
    """
    讲一个列表的图像（大小不一）保存为一个单独的tensor。
    对于不同大小的图像，会将图像padding到相同的大小，并将其每一张图像的原始大小保存到一个字段中。
    ...
    def __init__(self, tensor: torch.Tensor, image_sizes: List[Tuple[int, int]]):
        ...
        Arguments:
            tensor (Tensor):
                of shape (N, H, W) or (N, C_1, ..., C_K, H, W) where K >= 1
            image_sizes (list[tuple[int, int]]):
                Each tuple is (h, w). It can be smaller than (H, W) due to padding.
        """
        self.tensor = tensor
