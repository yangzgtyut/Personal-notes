...
class Boxes:
    ...
    def __init__(self, tensor: torch.Tensor):
        ...
        if tensor.numel() == 0:  # torch.numel(): 返回张量中的元素总数
            # Use reshape, so we don't end up creating a new tensor that does not depend on the inputs (and consequently confuses jit)
    ...
    def area(self) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor:
                一个包含每一个box面积的向量
                a vector with areas of each box.
        """
        box = self.tensor
    ...
    def nonempty(self, threshold: float = 0.0) -> torch.Tensor:
        ...
        如果一个box的任一条边的大小都小于阈值，则将其视为空box
        A box is considered empty, if either of its side is no larger than threshold.

        Returns:
            Tensor:
                一个二进制的向量，代表每一个box是否为空
    ...
    def inside_box(self, box_size: BoxSizeType, boundary_threshold: int = 0) -> torch.Tensor:
        """
        Args:
            box_size (height, width):
                Size of the reference box.
            boundary_threshold (int):
                Boxes that extend beyond the reference box boundary by more than boundary_threshold are considered "outside".
...
def pairwise_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    """
    给定两个长度分别为N和M的boxes的列表，计算 N x M 个box对之间的IoU。
    box的顺序必须为 (xmin, ymin, xmax, ymax).
...
def matched_boxlist_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    """
    与boxlist_iou相似，但是只计算矩阵的对角元素，即只计算相匹配的边框的IoU
