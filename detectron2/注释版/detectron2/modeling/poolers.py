...
class ROIPooler(nn.Module):
    ...
    feature maps.
    支持特征金字塔，可选"ROIPool" 或 "ROIAlignV2" 或 "ROIPooling"
    """
    ...
    def __init__(
        ...
        Args:
            output_size (int, tuple[int] or list[int]):
                RoIpooler的输出的特征图尺寸，e.g. 14 x 14
                output size of the pooled region, e.g., 14 x 14. If tuple or list is given, the length must be 2.
            scales (list[float]):
                The scale for each low-level pooling op relative to the input image. For a feature map with stride s relative to the input image, scale is defined as a 1 / s. The stride must be power of 2.
                * 多尺度必须构成金字塔：When there are multiple scales, they must form a pyramid, i.e. they must be a monotically decreasing geometric sequence with a factor of 1/2.
            sampling_ratio (int):
                ROIAlign的参数
                The `sampling_ratio` parameter for the ROIAlign op.
            pooler_type (string):
                Name of the type of pooling operation that should be applied.
                For instance, "ROIPool" or "ROIAlignV2".
            canonical_box_size (int):
                A canonical box size in pixels (sqrt(box area)). The default is heuristically defined as 224 pixels in the FPN paper (based on ImageNet pre-training).
            canonical_level (int):
    ...
    def forward(self, x: List[torch.Tensor], box_lists):
        ...
        Args:
            x (list[Tensor]):
                A list of feature maps of NCHW shape, with scales matching those
